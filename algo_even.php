    ...
    for ($k = 0; $k < 6; $k++) {
        for ($j = 0; $j < 6; $j++) {
            ...
            if (isset($teachers[$i]->days[0][$k][$j])) {
                ...
                $teachers[$i]->days[0][$k][$j] = "-";			//0 for even sem, 1 for odd sem
            }
        }
    }
}
...
for ($i = 0; $i < $teachers_count; $i++) {
    ...
    for ($j = 0; $j < 6; $j++) {
        $query = "UPDATE" . $database_name . " SET  period1= '" . $teachers[$i]->days[0][$j][0] . "',
period2='" . $teachers[$i]->days[0][$j][1] . "', 
period3='" . $teachers[$i]->days[0][$j][2] . "',
period4='" . $teachers[$i]->days[0][$j][3] . "', 
period5='" . $teachers[$i]->days[0][$j][4] . "', 
period6='" . $teachers[$i]->days[0][$j][5] . "'
 WHERE day='" . $days[$j] . "' ";
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"), $query);
    }
}
